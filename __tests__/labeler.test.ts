...
describe('mergeLabels, sync config', () => {
  ...
  it('rhs empty should join', function () {
    ...
    expect(mergeLabels([], config).sort()).toEqual(['a', 'b', 'c', 'no-sync2'].sort());
  });
  ...
  it('non empty should join', function () {
    ...
    expect(mergeLabels(['d', 'e', 'f'], config).sort()).toEqual(['a', 'b', 'c', 'd', 'e', 'f', 'no-sync1'].sort());
  });
  ...
  it('should dedupe', function () {
    ...
    expect(mergeLabels(['c', 'b', 'f'], config).sort()).toEqual(['a', 'b', 'c', 'f', 'no-sync2'].sort());
  });
});
